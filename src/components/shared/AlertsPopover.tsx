...
const MOCK_ALERTS: Alert[] = [
    ...
    {
        id: '3',
        title: 'Objectif de marge atteint',
        category: 'Performance',
        description: 'La marge brute mensuelle a atteint 28,5%, dépassant l\'objectif de 25%',
    ...
    {
        id: '4',
        title: 'Stock faible - Produit Ref #422',
        category: 'Logistique',
        description: 'Le niveau de stock pour ce produit est inférieur au seuil d\'alerte de 50 unités',
    ...
    },
    {
        ...
        category: 'Finance',
        description: 'Le budget marketing pour le mois en cours a dépassé les prévisions de 12%',
        ...
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="icon" className="relative text-slate-500 hover:text-slate-700">
                    ...
                    {unreadCount > 0 && (
                        <span className="absolute top-1 right-2 h-4 w-4 bg-red-500 rounded-full border border-white text-[10px] text-white flex items-center justify-center font-bold">
                            {unreadCount}
                        </span>
                    )}
                </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent className="w-[600px] p-0 mr-4 mt-2 shadow-xl border-slate-200" align="end">
                    ...
                    <Tabs defaultValue="toutes" className="flex-1 flex flex-col">
                        <div className="px-4 pt-2 border-b">
                            <TabsList className="bg-transparent h-10 p-0 gap-2 overflow-x-auto no-scrollbar justify-start">
                                ...
                                <TabsTrigger value="critiques" className="rounded-full data-[state=active]:bg-blue-900 data-[state=active]:text-white data-[state=inactive]:bg-slate-100 data-[state=inactive]:text-slate-500 h-8 px-4 text-xs font-medium">
                                    Critiques ({MOCK_ALERTS.filter(a => a.status === 'critical').length})
                                </TabsTrigger>
                                <TabsTrigger value="tresorerie" className="rounded-full data-[state=active]:bg-blue-900 data-[state=active]:text-white data-[state=inactive]:bg-slate-100 data-[state=inactive]:text-slate-500 h-8 px-4 text-xs font-medium">
                                    Trésorerie ({MOCK_ALERTS.filter(a => a.category === 'Trésorerie').length})
                                </TabsTrigger>
                            </TabsList>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto">
                            ...
                            <TabsContent value="non-lues" className="m-0 border-none outline-none">
                                {MOCK_ALERTS.filter(a => !a.isRead).map((alert) => (
                                    <AlertItem key={alert.id} alert={alert} />
                                ))}
                            </TabsContent>
                            {/* Other tabs follow same pattern if needed */}
                        </div>
                    </Tabs>

                    {/* Footer */}
                    <div className="p-3 border-t bg-slate-50/50 flex items-center justify-between text-xs text-slate-500">
                        <span>{MOCK_ALERTS.length} alerte(s) affichée(s)</span>
