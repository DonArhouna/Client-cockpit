                ...
                {barData.slice(0, 3).map((item) => (
                    <div key={item.name} className="flex flex-col">
                        <div className="flex justify-between text-[10px] mb-0.5">
                            ...
                            <span className="font-bold text-slate-700 dark:text-slate-300">{item.value.toLocaleString()} {currencySymbol}</span>
                        </div>
        ...
        <div className="flex flex-col h-full w-full">
            <div className="flex items-center justify-end mb-4 z-10 relative">
                <div className="flex items-center gap-4 text-sm">
                    <div className="flex items-center bg-slate-100 dark:bg-slate-800 rounded-md p-0.5 border dark:border-slate-700">
                        <button className="px-3 py-1 bg-white dark:bg-slate-900 shadow-sm rounded-sm font-medium text-slate-700 dark:text-slate-200">Âge des créances</button>
                    </div>
                    <Button variant="outline" size="sm" className="h-8 gap-2 dark:border-slate-700 dark:hover:bg-slate-800">
                ...
                <div className="flex-1 min-h-[200px]">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={barData} margin={{ top: 20, right: 0, left: -20, bottom: 0 }}>
                            ...
                            <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: axisColor, fontSize: 11 }} dy={10} />
                            <YAxis
                                axisLine={false}
                                tickLine={false}
            ...
            <div className="grid grid-cols-3 mt-6 pt-4 border-t dark:border-slate-800 text-sm">
                <div>
                    <div className="text-slate-500 dark:text-slate-500 text-xs mb-1">Moyenne</div>
                    <div className="font-semibold text-slate-800 dark:text-slate-200">
                        {totalValue > 0 ? (totalValue / barData.length).toLocaleString(undefined, {maximumFractionDigits: 0}) : 0} {currencySymbol}
                    </div>
                </div>
                <div>
                    <div className="text-slate-500 dark:text-slate-500 text-xs mb-1">Total créances</div>
                    <div className="font-semibold text-slate-800 dark:text-slate-200">{totalValue.toLocaleString()} {currencySymbol}</div>
                </div>
