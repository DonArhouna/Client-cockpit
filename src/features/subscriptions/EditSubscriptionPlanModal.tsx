...
export function EditSubscriptionPlanModal({ open, onOpenChange, plan }: EditSubscriptionPlanModalProps) {
  ...
  useEffect(() => {
    if (plan) {
      form.reset({
        ...
        maxKpis: plan.maxKpis ?? undefined,
        maxWidgets: plan.maxWidgets ?? undefined,
        hasNlq: plan.hasNlq ?? false,
  ...
  const mutation = useMutation({
    mutationFn: (values: FormValues) => subscriptionPlansApi.update(plan!.id, values),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['subscription-plans'] });
      toast({
        title: t('common.success'),
        description: t('subscriptionPlans.updateSuccess'),
      });
      onOpenChange(false);
    },
    onError: (error: any) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[540px]">
        <DialogHeader>
          <DialogTitle>{t('subscriptionPlans.edit')}</DialogTitle>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex items-center gap-2 space-y-0">
                    ...
                    <FormLabel className="font-normal">{t('subscriptionPlans.active')}</FormLabel>
                  </FormItem>
                )}
              />
            </div>
